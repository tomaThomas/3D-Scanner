...
dZ=0                         #Abstand vom Linienlaser zum Mittelpunkt des Drehtellers
d0=0                         #Abstand vom Linienlaser zum Objekt auf der Projektionslinie, das in der linksäußersten Pixelspalte der Kamera erscheint
b=0                          #Abstand vom Linienlaser zur Kamera
...
f=0                          #Brennweite der Kamera
d = f * np.tan(alphaz-alpha0)#halber Durchmesser des Sensors
M = 0                        #Anzahl Spalten pro Bild
N = 0                        #Anzahl Zeilen im Bild
...
def calculateCoordinates(pixel_coordinates,angle):
    ...
    y = (pixel_coordinates[1] * c) + ((N//2*c - (pixel_coordinates[1]*c ))/distance_cam_center)* (distance_cam_center-distance_point_cam)
    y = y - (N - jZ) * c
